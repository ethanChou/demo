...
#ifdef YUVSCALE_EXPORTS
#define JNAAPI extern "C" __declspec(dllexport)
#else
#define JNAAPI extern "C" __declspec(dllimport)
#endif
...
public:
	...
	virtual void invoke(char* args){
		printf("invoek : %s",args);
	}
};
...
JNAAPI int _stdcall yuv_yv12tobgr24(uint8_t* src,uint8_t* dst,int w,int h);

JNAAPI int _stdcall yuv_yv12tobgr24_fast(uint8_t* src,uint8_t* dst,int w,int h);
