...
{
	CLIENT clientproxy = *(CLIENT*)arg;
	SOCKET client =clientproxy.socket;


	//int clientsocket=(int)arg;
    //SOCKET client = (SOCKET)clientsocket;

	printf("IP_ADDRESS:%s, SOCKET:%d\n", clientproxy.ip, clientproxy.socket);
    char buf[1024];
    size_t numchars;
    char method[255];
    char url[255];
        ...
        const char *method, const char *query_string)
{
    ...
    //            recv(client, &c, 1, 0);
    //            send(cgi_input[1], &c, 1);
    //        }
    //    while (recv(cgi_output[0], &c, 1) > 0)
    //        send(client, &c, 1, 0);
...
int main(void)
{
    ...
    while (1)
    {
        ...
        /* accept_request(&client_sock); */
         ...
         strncpy(client->ip, inet_ntoa(client_name.sin_addr), 16);

		if (pthread_create(&newthread , NULL, accept_request, (void *)client) != 0)
