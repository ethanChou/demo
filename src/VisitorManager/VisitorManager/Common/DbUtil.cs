...
namespace VisitorManager.Common
{
    public class DbUtil
    {
        ...
        {
            using (IDbConnection _conn = GetSqlConnection(_sqlConnectionStr))
            {
                ...
                try
                {
                    ...
                    if (Int32.Parse(_conn.ExecuteScalar(querystr).ToString()) > 0)
                    {
                        return true;
                    }
                }
        ...
        public static bool UpdateModel<T>(T model, string primaryKey )
        {
            using (IDbConnection _conn = GetSqlConnection(_sqlConnectionStr))
            {
                try
                {
                    ...
                    return _conn.Execute(updatestr, model) > 0;
                }
                catch (Exception ex)
                {
                    return false;
        ...
        private static SQLiteConnection GetSqlConnection(string sqlConnectionString)
        {
            if (string.IsNullOrEmpty(sqlConnectionString))
                throw new InvalidCastException("sqlConnectionString is not null or empty,please call init method before using ");
